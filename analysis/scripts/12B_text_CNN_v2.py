...
def create_nahuatl_word_list(csv_file_path):
    ...
    try:
        ...
        print(
            f"Error: Nahuatl names CSV not found at {csv_file_path}. Proceeding with an empty list."
        )
    except KeyError:
        print(
            f"Error: 'official_name' column not found in {csv_file_path}. Proceeding with an empty list."
        )
...
def apply_idf_to_weights(
    raw_weights: dict[str, float], idf_scores: dict[str, float]
) -> dict[str, float]:
    """
...
if not NAHUATL_WORD_LIST:
    print(
        "WARNING: The Nahuatl word filter list is empty. This may lead to incorrect words in the word clouds."
    )
else:
    print(
        f"INFO: Generated a Nahuatl word list with {len(NAHUATL_WORD_LIST)} words for filtering."
    )

# --- NEW: Global IDF Calculation ---
print("\n--- Calculating global IDF scores from all aggregated texts ---")
all_texts_for_idf = (
    master_texts_df["aggregated_english_text"].dropna().tolist()
    + master_texts_df["aggregated_spanish_text"].dropna().tolist()
)
    ...
    if (
        ...
        for sample_idx in range(len(y_original_for_skf)):
            ...
            if len(words) > len(X_embeddings_padded[0]):
                print(
                    f"Warning: Text for sample {sample_idx} has more words ({len(words)}) than the max sequence length ({len(X_embeddings_padded[0])}). Skipping to avoid misalignment."
                )
                continue
                ...
                if has_label:
                    ...
                    if heatmap is not None:
                        ...
                        for word, score in zip(words, heatmap_resized):
                            ...
                            if (
                                cleaned_word
                                and cleaned_word not in stopword_set
                                and cleaned_word not in NAHUATL_WORD_LIST
                            ):
        ...
        for class_name, word_scores in class_word_importances.items():
            if word_scores:
                ...
                re_weighted_scores = apply_idf_to_weights(
                    word_scores, global_idf_scores
                )
                ...
                figure_filename = (
                    wordcloud_figure_output_dir
                    / f"wordcloud_gradcam_idf_{class_name}.png"
                )
